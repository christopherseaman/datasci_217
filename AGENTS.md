...
- Homework is pass/fail, and students test it themselves: each homework handout ships a byte-identical copy of its course-owned checks, so `python3 check_assignment.py` gives the full score and says what to fix. Exams (Assignments 05 and 11) are the exception: their handouts ship shape checks only, and values are checked on GitHub. Grade generously: whitespace never costs points, and one mistake should cost the points for what it got wrong, not a whole task.
