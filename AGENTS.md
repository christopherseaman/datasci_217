...
- Media is uploaded to Notion from the local files, never linked to GitHub. Immediately before publishing a page, run `python3 scripts/notion_media.py <manifest.json> <page.md>` to upload its images, then pass `--media-manifest <manifest.json>` to `scripts/notion_publish.py` so every image becomes a `file-upload://` source. Uploads expire about an hour after they are created if nothing attaches them, so upload and publish in one sitting. An image link written as `![Caption](file-upload://ID)` is correct; wrapping the upload tag inside the link, as in `![Caption](<image src="file-upload://ID"></image>)`, stores a dead text link instead of a picture.
- Publish with `python3 scripts/notion_push.py <page.md>`, which does the whole sequence and refuses to continue when a step fails: page the child blocks to the end, upload the media, build the payload, check every child page survived into it, write the page, attach the images, then re-read and verify. `--dry-run` stops before writing. Verify any time with `python3 scripts/notion_check_pages.py`, which reports broken images across every mapped page and exits non-zero.
- A child page left out of the payload does not silently disappear: `ntn pages edit` HANGS on it until killed, and the page can stay wedged so later edits hang too. A normal edit returns in about a second, so a publish that stalls means a missing `<page>` tag, not a slow API. Reproduced 2026-09-23 on throwaway pages; the child page was never deleted in any attempt.
- Whether that `file-upload://` source resolves depends on how the page is written. The Notion page-update tool resolves it; `ntn pages edit` does NOT, and silently stores each image as an external block with an empty URL, which renders as a broken image. After publishing a page with the CLI, run `python3 scripts/notion_attach_media.py <page-id> <page.md>`, which matches each placeholder block to its local file by caption, uploads it, and swaps a real image block into its place.
- Verify images after publishing rather than trusting the payload: an image block counts as broken when `block["image"][block["image"]["type"]]["url"]` is empty. Checking only that the key exists passes a broken block, because the empty external source is still a source.
