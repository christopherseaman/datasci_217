...
Work in the Classroom 50 repository provisioned for you. Use the supported POSIX-style shell from Lecture 01 and terminal-executed `.py` files. Do not create a notebook.
...
A complete submission ends with `All public checks passed.` Classroom 50 may run the ten public facade tests with managed pytest. Production tests, if used, independently implement the same written contract in the centrally managed Classroom 50 bundle; they do not import or trust the editable checker in a student repository. Published grader code is discoverable and is not described as secret.
