...
EXPECTED_ENVIRONMENT_OUTPUT = ("Python: 3.12.13", "NumPy: 2.0.2")
...
def check_runtime_records_and_probe(root: Path) -> None:
    _assert((root / ".python-version").read_text(encoding="utf-8") == EXPECTED_PYTHON_RECORD, "Replace .python-version with exactly `3.12.13` and one final newline.")
    _assert((root / "requirements.txt").read_text(encoding="utf-8") == EXPECTED_REQUIREMENTS, "Replace requirements.txt with exactly `numpy==2.0.2` and one final newline.")
