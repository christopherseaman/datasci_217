...
- Status: Lectures 04–11 are edited locally (README, BONUS, and media) per the approved plan, reviewed, cross-checked, and fixed up; nothing is committed. Lectures 01–03 have a plan (second half of `LECTURE_PLAN.md`) awaiting instructor approval; the only 01–03 change so far is Lecture 01's wrong-indentation fence ported from Notion.
...
- Open lecture items:
    ...
    - Title-line convention: Lectures 02–03 and 09–11 use `# Title` for the title line and `title_line`; 04–08 use a plain title line. Both publish correctly; pick one.
    - BONUS cleanup across all lectures: bold `**Reference:**`/`**Example:**` labels instead of headings, legacy `np.random` calls, and a few examples with undefined variables (07 and 09 BONUS).
...
- Notion images: where Notion holds an uploaded image that differs from local Markdown (Lecture 05's IQR outlier figure, Lecture 09's xkcd 2048 and 2289), keep the Notion upload when publishing.
