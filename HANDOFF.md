...
- Open lecture items:
    ...
    - Break balance that only demo changes can fix (measured 2026-09-22 with `python3 scripts/lecture_metrics.py --blocks`): 04 at 29/49/21, 08 at 48/23/29, 10 at 27/60/13. Every legal break position was checked against the demo files and each one would leave a demo practicing untaught material; the unblocking demo edits are in the deferred list below. Lecture 01 was rebalanced to 23/18/28/31 by moving comparison operators into Python Basics; its block 2 (shell) stays at 18% because every block-1 topic is required by Demo 1 and every block-3 topic by Demo 3, so it can only grow by authoring new shell material.
    - Six lectures now have the intended pacing (a slightly larger first block, then roughly even): 02, 03, 05, 06, 07, 09.
...
- Deferred to the demo pass (after the lectures are settled), including factual demo errors. After the lecture edits, each lecture's remaining demo items that use material not taught before their break are listed in `scratch/review/apply_run1.json` (`demo_alignment` for each lecture).
    - 04-00 (pacing): the best split is 29/38/33, with break 2 after `## Filtering Rows with a Boolean Mask`. Blocked because `demo2_pandas_basics.md` adds the derived column `measurement_table["change"]` and sorts with `by=["change", "record_id"]`, taught in `## Adding Columns` and `## Sorting Rows`. Move those two demo sections into demo 3, or teach the two subtopics before the break.
    - 08-00 (pacing): the coherent regrouping is 32/38/29 — move `# Which Groups Appear in a Summary` up after `# Basic GroupBy Operations`, put break 1 there, and leave break 2 after pivot tables. Blocked because `demo1_groupby_operations.md` "Part 2: Advanced GroupBy Operations" makes 5 `transform`, 3 `filter`, and 4 `apply` calls, all taught in `# GroupBy Result-Shape Choices`. Move that part into `demo2_pivot_tables.md` ahead of its Part 1, and add a group-coverage section to demo 1 (`pd.Categorical`, `observed=`, `dropna=`), which no demo currently practices.
    - 10-00 (pacing): the fix is 28/37/35 — promote `## Random Forest` and `## Permutation Importance` into their own `#` topic and move break 2 to sit before it, after `## Freeze, Then Test Once`. Blocked because `demo2_ml_boosting.md` Parts 5, 6, 9, and 10 fit a RandomForest, fit XGBoost, read `feature_importances_`, run `permutation_importance`, and use early stopping. Move those parts into `demo3_deep_learning.md`, whose Part 8 already refits LogisticRegression, RandomForest, and XGBoost to compare against the network. Retarget the sentence at `10/README.md` "Demo 2's final part practices the refit and single test evaluation." if demo 2's parts are renumbered.
    - 01-00 (pacing, optional): comparison operators are now taught in block 3 but first practiced in Demo 4. Add a one-line checkpoint to Demo 3 (for example `print("score at least 85:", average_score >= 85)`), which would use only block-3 material.
