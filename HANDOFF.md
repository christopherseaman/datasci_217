...
- Broken images in Notion: 16 lecture images are dangling `file-upload://` links (Lectures 05, 07, 08, 09, 10, 11), listed with block ids in `scratch/review/bad_images.txt` (scanner: `scratch/review/notion_bad_images.py`). Local Markdown and image files are fine; publishing, or a targeted block repair through the `ntn` CLI, would fix them.
