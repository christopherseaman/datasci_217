...
def grade_submission(submission_root: str | Path) -> dict:
    ...
    for row in rows:
        ...
        tests.append({"test-name": row["name"], "passed": row["passed"], "score": score,
                      "max-score": row["points"], "detail": row["detail"]})
    return {"schema": "datasci217/grading-result/v1", "score": sum(test["score"] for test in tests), "max-score": sum(POINTS), "tests": tests}
