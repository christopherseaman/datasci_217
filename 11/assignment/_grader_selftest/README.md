...
copyable reference pipeline. The central Classroom50 grader independently applies
exact deterministic reference checks. Central-grader discoverability and packaging
are controlled by the course platform rather than by the student starter.
...
`requirements.txt`, invokes the central grader, and writes an exact
`classroom50/result/v1` `result.json`. Learner artifact failures exit 0 with all
