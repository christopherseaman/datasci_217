...
The report covers the 85 points graded from files; the other 15 come from human review of the notebook, as the handout README's Completion contract describes. The command exits 0 only when every check passes, so judge a run by its JSON, not its exit status.
...
| `cleaned_people.csv: <column>`, one check for each of the 7 columns | 35 | 5 each, in proportion to the 11 records right in that column, rounded down |
| `decision_log.csv: decisions` | 8 | 1 per decision whose field, issue, and action match |
| `decision_log.csv: reason`, `source`, `source_sha256`, `rows_before`, `rows_after` | 2, 1, 2, 2, 2 | In proportion to the decision rows with the right value, rounded down |
