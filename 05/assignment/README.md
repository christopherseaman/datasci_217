...
Dataset: `data/clinical_trial_raw.csv` (10,000 patients, 18 variables) - *Generate this by running `python3 generate_data.py`*
...
This creates `data/clinical_trial_raw.csv` with 10,000 patients and 18 variables.
...
6. Save directory listing to `reports/directory_structure.txt` using `tree`
...
2. **Have shebang** `#!/bin/bash`
3. **Run Q2 script** - Execute `python3 q2_process_metadata.py`
4. **Run Q4-Q7 notebooks** - Execute notebooks in order using `jupyter nbconvert --execute --to notebook`
5. **Handle errors gracefully** - Use `||` operator to check exit codes and stop on failure
6. **Generate pipeline log** - Save execution status to `reports/pipeline_log.txt`
...
2. NOTE: Q3 script is not run directly, instead functions from it are imported in Q4-Q7
3. Handle errors gracefully (stop on failure)
4. Log all execution steps to the pipeline log
