...
List the patient ID of every patient with at least one usable reading at or above your cutoff, one per line, each patient once. Order does not matter, the `mmHg` unit on the cutoff is optional, and any other line — a heading, a blank line, a row of dashes — is ignored.
