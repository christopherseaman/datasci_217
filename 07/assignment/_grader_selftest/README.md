...
They provide an independent Classroom50 grader candidate and adversarial release
harness. The grader never imports or invokes `check_assignment.py`.

Classroom50 invokes the standard-library `autograder.py` bootstrap with plain
...
artifacts. Human review then uses Classroom50's official `REVIEW_URL` to inspect
