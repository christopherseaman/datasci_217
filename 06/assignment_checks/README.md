# Assignment 06 checks (course-owned)

These are the checks that decide the Assignment 06 grade. They read the five
CSV files a submission saves in `output/` and compare them with values taken
from the supplied specimen, clinic, courier, and blood pressure files.
`06/assignment/.github/workflows/tests.yml` downloads them on every student
push from `christopherseaman/datasci_217@main:06/assignment_checks/` (its
...
files in `06/assignment/data/`. A change to the check list or to `POINTS`
